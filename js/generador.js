...
function longitudValida(longitud){
    if (isNaN(longitud) || longitud == ""){
        mostrarAlerta("No has ingresado un número de caracteres válido.", "error");
        return false;
    } else if (isNaN(longitud) || longitud < 8){
        mostrarAlerta("La contraseña debe ser mayor o igual a 8.", "error");
        return false;
    }
    return true;
};
...
function copiarContraseña(){
    if (!contraseña.value){
        mostrarAlerta("No hay contraseña ninguna para copiar.", "error");
        return;
    }
    contraseña.select();
